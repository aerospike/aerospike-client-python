...
#define BYTES_POOLS(dynamic_pool) (((as_dynamic_pool *)dynamic_pool)->pools)
...
static inline void dynamic_pool_init(as_dynamic_pool *dynamic_pool,
                                     as_error *err)
{
    BYTES_POOLS(dynamic_pool) = (as_bytes **)malloc(
    ...
    BYTES_POOLS(dynamic_pool)
    [0] = (as_bytes *)malloc(sizeof(as_bytes) * AS_DYNAMIC_POOL_BLOCK_SIZE);
                                       ...
                                       as_error *err)
{
    ...
    if (BYTES_POOL_CNT(dynamic_pool) % AS_DYNAMIC_POOL_POINTER_BLOCK_SIZE ==
        0) {
        BYTES_POOLS(dynamic_pool) = (as_bytes **)realloc(
            BYTES_POOLS(dynamic_pool), (BYTES_POOL_CNT(dynamic_pool) +
    ...
    BYTES_POOLS(dynamic_pool)
    [BYTES_POOL_CNT(dynamic_pool)] =
        (as_bytes *)malloc(sizeof(as_bytes) * BYTES_BLOCK_SIZE(dynamic_pool));
...
static inline void pool_destroy(as_dynamic_pool *dynamic_pool, bool freeBytes)
{
    uint32_t delete_pool_size = AS_DYNAMIC_POOL_BLOCK_SIZE;
    for (uint32_t pool_num = 0; pool_num < BYTES_POOL_CNT(dynamic_pool);
         pool_num++) {
        if (freeBytes) {
