...
PyObject * AerospikeClient_SetXDRFilter(AerospikeClient * self, PyObject * args, PyObject * kwds)
{
	...
	if (PyArg_ParseTupleAndKeywords(args, kwds, "OOO|O:set_xdr_filter", kwlist, &py_data_center, &py_namespace, &py_expression_filter, &py_policy) == false) {
		return NULL;
	}
    ...
    sprintf(request_str_p, fmt_str, data_center_str_p, namespace_str_p, base64_filter);

    as_status status = AEROSPIKE_OK;
	Py_BEGIN_ALLOW_THREADS
	status = aerospike_info_any(self->as, &err, info_policy_p, request_str_p, &response_p);
