...
static as_status get_bit_policy(bool validate_keys, as_error *err,
                                PyObject *op_dict, as_bit_policy *policy);
                                   ...
                                   int serializer_type)
{
    ...
    if (get_bit_policy(self->validate_keys, err, op_dict, &bit_policy) !=
        AEROSPIKE_OK) {
        return err->code;
    }
