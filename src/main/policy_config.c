...
as_status set_subpolicies(as_error *err, as_config *config,
                          PyObject *py_policies_dict, int validate_keys)
{
    ...
    set_policy_status = as_policy_read_set_from_pyobject(
        NULL, &err, read_policy, &config->policies.read, false, validate_keys);
    as_error_reset(&err);
    ...
    set_policy_status = as_policy_apply_set_from_pyobject(
        NULL, &err, py_apply_policy, &config->policies.apply, false,
        validate_keys);
    as_error_reset(&err);
    if (set_policy_status != AEROSPIKE_OK) {
        return set_policy_status;
    ...
    set_policy_status =
        set_batch_policy(&err, &config->policies.batch_parent_write,
                         batch_parent_write_policy, validate_keys);
    if (set_policy_status != AEROSPIKE_OK) {
        return set_policy_status;
    }
                            ...
                            PyObject *py_policy, int validate_keys)
{
    ...
    if (validate_keys) {
        int retval = does_py_dict_contain_valid_keys(
            err, py_policy, py_remove_policy_valid_keys, true);
        if (retval != 1) {
            return AEROSPIKE_ERR_PARAM;
        }
    }
                           ...
                           PyObject *py_policy, int validate_keys)
{
    ...
    if (validate_keys) {
        int retval = does_py_dict_contain_valid_keys(
            err, py_policy, py_query_policy_valid_keys, true);
        if (retval != 1) {
            return AEROSPIKE_ERR_PARAM;
        }
    }
                             ...
                             PyObject *py_policy, int validate_keys)
{
    ...
    if (validate_keys) {
        int retval = does_py_dict_contain_valid_keys(
            err, py_policy, py_operate_policy_valid_keys, true);
        if (retval != 1) {
            return AEROSPIKE_ERR_PARAM;
        }
    }
                           ...
                           PyObject *py_policy, int validate_keys)
{
    ...
    if (validate_keys) {
        int retval = does_py_dict_contain_valid_keys(
            err, py_policy, py_batch_policy_valid_keys, true);
        if (retval != 1) {
            return AEROSPIKE_ERR_PARAM;
        }
    }
                           ...
                           PyObject *py_policy, int validate_keys)
{
    ...
    if (validate_keys) {
        int retval = does_py_dict_contain_valid_keys(
            err, py_policy, py_admin_policy_valid_keys, true);
        if (retval != 1) {
            return AEROSPIKE_ERR_PARAM;
        }
    }
                                 ...
                                 PyObject *py_policy, int validate_keys)
{
    ...
    if (validate_keys) {
        int retval = does_py_dict_contain_valid_keys(
            err, py_policy, py_batch_apply_policy_valid_keys, true);
        if (retval != 1) {
            return AEROSPIKE_ERR_PARAM;
        }
    }
                                 ...
                                 PyObject *py_policy, int validate_keys)
{
    ...
    if (validate_keys) {
        int retval = does_py_dict_contain_valid_keys(
            err, py_policy, py_batch_write_policy_valid_keys, true);
        if (retval != 1) {
            return AEROSPIKE_ERR_PARAM;
        }
    }
                                  ...
                                  PyObject *py_policy, int validate_keys)
{
    ...
    if (validate_keys) {
        int retval = does_py_dict_contain_valid_keys(
            err, py_policy, py_batch_remove_policy_valid_keys, true);
        if (retval != 1) {
            return AEROSPIKE_ERR_PARAM;
        }
    }
