...
/* Predexp constructor function which takes a single char* argument */
as_status
add_no_arg_predicate(as_query* query, PyObject* predicate, as_error* err,
...
as_status add_rec_device_size(as_query* query, PyObject* predicate, as_error* err);
as_status add_rec_last_update(as_query* query, PyObject* predicate, as_error* err);
as_status add_rec_void_time(as_query* query, PyObject* predicate, as_error* err);
as_status add_rec_digest_modulo(as_query* query, PyObject* predicate, as_error* err);
as_status add_integer_equal(as_query* query, PyObject* predicate, as_error* err);
as_status add_integer_unequal(as_query* query, PyObject* predicate, as_error* err);
as_status add_integer_greater(as_query* query, PyObject* predicate, as_error* err);
as_status add_integer_greatereq(as_query* query, PyObject* predicate, as_error* err);
as_status add_integer_less(as_query* query, PyObject* predicate, as_error* err);
as_status add_integer_lesseq(as_query* query, PyObject* predicate, as_error* err);
as_status add_string_equal(as_query* query, PyObject* predicate, as_error* err);
as_status add_string_unequal(as_query* query, PyObject* predicate, as_error* err);
as_status add_string_regex(as_query* query, PyObject* predicate, as_error* err);
as_status add_geojson_within(as_query* query, PyObject* predicate, as_error* err);
as_status add_geojson_contains(as_query* query, PyObject* predicate, as_error* err);
as_status add_list_iterate_or(as_query* query, PyObject* predicate, as_error* err);
as_status add_list_iterate_and(as_query* query, PyObject* predicate, as_error* err);
as_status add_mapkey_iterate_or(as_query* query, PyObject* predicate, as_error* err);
as_status add_mapkey_iterate_and(as_query* query, PyObject* predicate, as_error* err);
as_status add_mapval_iterate_or(as_query* query, PyObject* predicate, as_error* err);
as_status add_mapval_iterate_and(as_query* query, PyObject* predicate, as_error* err);

/* MACROS For simple tuple return functions */
#define OneArgPredExpBuilderFunc(_name, _pyfunc_name, _predexp_code) static PyObject * AerospikePredExp_ ## _name(PyObject * self, PyObject * param) \
{\
	return Py_BuildValue("(iO)", _predexp_code, param);\
}
/* Functions which return a tuple of the type (num_value,) */
#define NoArgPredExpBuilderFunc(_name, _pyfunc_name, _predexp_code) static PyObject * AerospikePredExp_ ## _name(PyObject * self, PyObject * unused) \
{\
	return Py_BuildValue("(i)", _predexp_code);\
}

#define OneArgPredExpFunctionEntry(_name, _pyfunc_name, _doc_var) {#_pyfunc_name, (PyCFunction) AerospikePredExp_ ## _name, METH_O, _doc_var}
#define NoArgPredExpFunctionEntry(_name, _pyfunc_name, _doc_var) {#_pyfunc_name, (PyCFunction) AerospikePredExp_ ## _name, METH_NOARGS, _doc_var}
...
AerospikeQuery * AerospikeQuery_Predexp(AerospikeQuery * self, PyObject * args) {
	...
	if (predicate_count == 0) {
		as_error_update(&err, AEROSPIKE_ERR_PARAM, "Predicates list must not be empty");
		goto CLEANUP;
	}
...
as_status add_predexp(as_query* query, PyObject* predicate, as_error* err) {
	...
	if (!predicate || !PyTuple_Check(predicate) || PyTuple_Size(predicate) < 1) {
	...
	if (!py_pred_type || !PyInt_Check(py_pred_type)) {
		return as_error_update(err, AEROSPIKE_ERR_PARAM, "Invalid predicate type");
	}
...
as_status add_and(as_query* query, PyObject* predicate, as_error* err) {
	if (PyTuple_Size(predicate) != 2) {
		return as_error_update(err, AEROSPIKE_ERR_PARAM, "Invalid and predicate");
	}
...
as_status add_or(as_query* query, PyObject* predicate, as_error* err){
	if (PyTuple_Size(predicate) != 2) {
		return as_error_update(err, AEROSPIKE_ERR_PARAM, "Invalid or predicate");
	}
...
as_status add_integer_val(as_query* query, PyObject* predicate, as_error* err){
	if (PyTuple_Size(predicate) != 2) {
		return as_error_update(err, AEROSPIKE_ERR_PARAM, "Invalid integer val predicate");
	}
...
as_status add_string_val(as_query* query, PyObject* predicate, as_error* err) {
	return add_single_string_arg_predicate(query, predicate, err, as_predexp_string_value, "string value");
}

as_status add_geojson_val(as_query* query, PyObject* predicate, as_error* err){
	return add_single_string_arg_predicate(query, predicate, err, as_predexp_geojson_value, "geojson value");
}

as_status add_int_bin(as_query* query, PyObject* predicate, as_error* err) {
	return add_single_string_arg_predicate(query, predicate, err, as_predexp_integer_bin, "integer bin");
}

as_status add_string_bin(as_query* query, PyObject* predicate, as_error* err) {
	return add_single_string_arg_predicate(query, predicate, err, as_predexp_string_bin, "string bin");
}

as_status add_geo_bin(as_query* query, PyObject* predicate, as_error* err) {
	return add_single_string_arg_predicate(query, predicate, err, as_predexp_geojson_bin, "geojson bin");
}

as_status add_list_bin(as_query* query, PyObject* predicate, as_error* err) {
	return add_single_string_arg_predicate(query, predicate, err, as_predexp_list_bin, "list bin");
}

as_status add_map_bin(as_query* query, PyObject* predicate, as_error* err) {
	return add_single_string_arg_predicate(query, predicate, err, as_predexp_map_bin, "map bin");

}

as_status add_integer_var(as_query* query, PyObject* predicate, as_error* err) {
	return add_single_string_arg_predicate(query, predicate, err, as_predexp_integer_var, "integer var");
}

as_status add_string_var(as_query* query, PyObject* predicate, as_error* err) {
	return add_single_string_arg_predicate(query, predicate, err, as_predexp_string_var, "string var");
}

as_status add_geojson_var(as_query* query, PyObject* predicate, as_error* err) {
	return add_single_string_arg_predicate(query, predicate, err, as_predexp_geojson_var, "geojson var");
}

as_status add_rec_device_size(as_query* query, PyObject* predicate, as_error* err) {
	return add_no_arg_predicate(query, predicate, err, as_predexp_rec_device_size, "rec device size");
}

as_status add_rec_last_update(as_query* query, PyObject* predicate, as_error* err) {
	return add_no_arg_predicate(query, predicate, err, as_predexp_rec_last_update, "rec last update");
}

as_status add_rec_void_time(as_query* query, PyObject* predicate, as_error* err) {
	return add_no_arg_predicate(query, predicate, err, as_predexp_rec_void_time, "rec void time");
}

as_status add_rec_digest_modulo(as_query* query, PyObject* predicate, as_error* err) {
	if (PyTuple_Size(predicate) != 2) {
		return as_error_update(err, AEROSPIKE_ERR_PARAM, "Invalid digest modulo predicate");
	}
	PyObject* py_int_val = PyTuple_GetItem(predicate, 1);
	if (!py_int_val || !PyInt_Check(py_int_val)) {
...
as_status add_integer_equal(as_query* query, PyObject* predicate, as_error* err) {
	return add_no_arg_predicate(query, predicate, err, as_predexp_integer_equal, "integer equal");
}

as_status add_integer_unequal(as_query* query, PyObject* predicate, as_error* err) {
	return add_no_arg_predicate(query, predicate, err, as_predexp_integer_unequal, "integer unequal");
}

as_status add_integer_greater(as_query* query, PyObject* predicate, as_error* err) {
	return add_no_arg_predicate(query, predicate, err, as_predexp_integer_greater, "integer greater");
}

as_status add_integer_greatereq(as_query* query, PyObject* predicate, as_error* err) {
	return add_no_arg_predicate(query, predicate, err, as_predexp_integer_greatereq, "integer greatereq");
}

as_status add_integer_less(as_query* query, PyObject* predicate, as_error* err) {
	return add_no_arg_predicate(query, predicate, err, as_predexp_integer_less, "integer less");
}

as_status add_integer_lesseq(as_query* query, PyObject* predicate, as_error* err) {
	return add_no_arg_predicate(query, predicate, err, as_predexp_integer_lesseq, "integer lesseq");
}

as_status add_string_equal(as_query* query, PyObject* predicate, as_error* err) {
	return add_no_arg_predicate(query, predicate, err, as_predexp_string_equal, "string equal");
}

as_status add_string_unequal(as_query* query, PyObject* predicate, as_error* err) {
	return add_no_arg_predicate(query, predicate, err, as_predexp_string_unequal, "string unequal");

}

as_status add_string_regex(as_query* query, PyObject* predicate, as_error* err) {
	if (PyTuple_Size(predicate) != 2) {
		return as_error_update(err, AEROSPIKE_ERR_PARAM, "Invalid string regex predicate");
	}
	PyObject* py_int_val = PyTuple_GetItem(predicate, 1);
	if (!py_int_val || !PyInt_Check(py_int_val)) {
...
as_status add_geojson_within(as_query* query, PyObject* predicate, as_error* err){
	return add_no_arg_predicate(query, predicate, err, as_predexp_geojson_within, "geojson within");
}

as_status add_geojson_contains(as_query* query, PyObject* predicate, as_error* err) {
	return add_no_arg_predicate(query, predicate, err, as_predexp_geojson_contains, "geojson contains");
}

as_status add_list_iterate_or(as_query* query, PyObject* predicate, as_error* err) {
	return add_single_string_arg_predicate(query, predicate, err, as_predexp_list_iterate_or, "list_iterate_or");
}

as_status add_list_iterate_and(as_query* query, PyObject* predicate, as_error* err) {
	return add_single_string_arg_predicate(query, predicate, err, as_predexp_list_iterate_and, "list_iterate_and");
}

as_status add_mapkey_iterate_or(as_query* query, PyObject* predicate, as_error* err){
	return add_single_string_arg_predicate(query, predicate, err, as_predexp_mapkey_iterate_or, "mapkey_iterate_or");

}

as_status add_mapkey_iterate_and(as_query* query, PyObject* predicate, as_error* err){
	return add_single_string_arg_predicate(query, predicate, err, as_predexp_mapkey_iterate_and, "mapkey_iterate_and");
}

as_status add_mapval_iterate_or(as_query* query, PyObject* predicate, as_error* err){
	return add_single_string_arg_predicate(query, predicate, err, as_predexp_mapval_iterate_or, "mapval_iterate_or");
}
...
add_single_string_arg_predicate(as_query* query, PyObject* predicate, as_error* err,
	...
	if (string_and_pyuni_from_pystring(py_str_val, &py_uni, &c_var_name, err) != AEROSPIKE_OK) {
		return err->code;
	}

	if (!as_query_predexp_add(query, constructor(c_var_name))) {
		as_error_update(err, AEROSPIKE_ERR_PARAM, "Failed to add %s predicate", predicate_name);
	}
...
add_no_arg_predicate(as_query* query, PyObject* predicate, as_error* err,
	...
	if (!as_query_predexp_add(query, no_arg_constructor())) {
		return as_error_update(err, AEROSPIKE_ERR_PARAM, "Failed to add %s predicate", predicate_name);
	}
	return err->code;
}

/* Predicate functions */
OneArgPredExpBuilderFunc(PredexpAnd, predexp_and, AS_PREDEXP_AND)
OneArgPredExpBuilderFunc(PredexpOr, predexp_or, AS_PREDEXP_OR)
OneArgPredExpBuilderFunc(IntegerValue, integer_value, AS_PREDEXP_INTEGER_VALUE)
OneArgPredExpBuilderFunc(StringValue, string_value, AS_PREDEXP_STRING_VALUE)
OneArgPredExpBuilderFunc(GeojsonValue, geojson_value, AS_PREDEXP_GEOJSON_VALUE)
OneArgPredExpBuilderFunc(IntegerBin, integer_bin, AS_PREDEXP_INTEGER_BIN)
OneArgPredExpBuilderFunc(StringBin, string_bin, AS_PREDEXP_STRING_BIN)
OneArgPredExpBuilderFunc(GeojsonBin, geojson_bin, AS_PREDEXP_GEOJSON_BIN)
OneArgPredExpBuilderFunc(ListBin, list_bin, AS_PREDEXP_LIST_BIN)
OneArgPredExpBuilderFunc(MapBin, map_bin, AS_PREDEXP_MAP_BIN)
OneArgPredExpBuilderFunc(IntegerVar, integer_var, AS_PREDEXP_INTEGER_VAR)
OneArgPredExpBuilderFunc(StringVar, string_var, AS_PREDEXP_STRING_VAR)
OneArgPredExpBuilderFunc(GeojsonVar, geojson_var, AS_PREDEXP_GEOJSON_VAR)
OneArgPredExpBuilderFunc(RecDigestModulo, rec_digest_modulo, AS_PREDEXP_REC_DIGEST_MODULO)
OneArgPredExpBuilderFunc(ListIterateOr, list_iterate_or, AS_PREDEXP_LIST_ITERATE_OR)
OneArgPredExpBuilderFunc(ListIterateAnd, list_iterate_and, AS_PREDEXP_LIST_ITERATE_AND)
OneArgPredExpBuilderFunc(MapkeyIterateOr, mapkey_iterate_or, AS_PREDEXP_MAPKEY_ITERATE_OR)
OneArgPredExpBuilderFunc(MapkeyIterateAnd, mapkey_iterate_and, AS_PREDEXP_MAPKEY_ITERATE_AND)
OneArgPredExpBuilderFunc(MapvalIterateOr, mapval_iterate_or, AS_PREDEXP_MAPVAL_ITERATE_OR)
OneArgPredExpBuilderFunc(MapvalIterateAnd, mapval_iterate_and, AS_PREDEXP_MAPVAL_ITERATE_AND)
/* No argument Predexp Functions */
NoArgPredExpBuilderFunc(PredexpNot, predexp_not, AS_PREDEXP_NOT)
NoArgPredExpBuilderFunc(RecDeviceSize, rec_device_size, AS_PREDEXP_REC_DEVICE_SIZE)
NoArgPredExpBuilderFunc(RecLastUpdate, rec_last_update, AS_PREDEXP_REC_LAST_UPDATE)
NoArgPredExpBuilderFunc(RecVoidTime, rec_void_time, AS_PREDEXP_REC_VOID_TIME)

NoArgPredExpBuilderFunc(IntegerEqual, integer_equal, AS_PREDEXP_INTEGER_EQUAL)
NoArgPredExpBuilderFunc(IntegerUnequal, integer_unequal, AS_PREDEXP_INTEGER_UNEQUAL)
NoArgPredExpBuilderFunc(IntegerGreater, integer_greater, AS_PREDEXP_INTEGER_GREATER)
NoArgPredExpBuilderFunc(IntegerGreaterEq, integer_greatereq, AS_PREDEXP_INTEGER_GREATEREQ)
NoArgPredExpBuilderFunc(IntegerLess, integer_less, AS_PREDEXP_INTEGER_LESS)
NoArgPredExpBuilderFunc(IntegerLessEq, integer_lesseq, AS_PREDEXP_INTEGER_LESSEQ)
NoArgPredExpBuilderFunc(StringEqual, string_equal, AS_PREDEXP_STRING_EQUAL)
NoArgPredExpBuilderFunc(StringUnequal, string_unequal, AS_PREDEXP_STRING_UNEQUAL)
NoArgPredExpBuilderFunc(GeojsonWithin, geojson_within, AS_PREDEXP_GEOJSON_WITHIN)
NoArgPredExpBuilderFunc(GeojsonContains, geojson_contains, AS_PREDEXP_GEOJSON_CONTAINS)
/* Predexp Method Docstrings */

/*String regex is special*/
static PyObject*
AerospikePredExp_PredexpStringRegex(PyObject* self, PyObject* args) {
	int flag = 0;
    ...
    for (int i = 0; i < arg_count; i++) {
    	...
    	if (PyInt_Check(py_flag)) {
    		...
    		PyErr_SetString(PyExc_TypeError, "Arguments to string_regex must be integers or longs");
    		return NULL;
    	}
    }
...
PyDoc_STRVAR(predexp_or_doc,
"predexp_or(item_count) -> (predexp_code, value)\n\
\n\
Creates a tuple to be used with the query.predexp method");
...
PyDoc_STRVAR(string_value_doc,
"string_value(value) -> (predexp_code, value)\n\
\n\
Creates a tuple to be used with the query.predexp method");
...
PyDoc_STRVAR(integer_bin_doc,
"integer_bin(value) -> (predexp_code, value)\n\
\n\
Creates a tuple to be used with the query.predexp method");

PyDoc_STRVAR(string_bin_doc,
"string_bin(value) -> (predexp_code, value)\n\
\n\
Creates a tuple to be used with the query.predexp method");

PyDoc_STRVAR(geojson_bin_doc,
"geojson_bin(value) -> (predexp_code, value)\n\
\n\
Creates a tuple to be used with the query.predexp method");


PyDoc_STRVAR(list_bin_doc,
"list_bin(value) -> (predexp_code, value)\n\
\n\
Creates a tuple to be used with the query.predexp method");

PyDoc_STRVAR(map_bin_doc,
"map_bin(value) -> (predexp_code, value)\n\
\n\
Creates a tuple to be used with the query.predexp method");

PyDoc_STRVAR(predexp_not_doc,
"predexp_not() -> (predexp_code,)\n\
\n\
Creates a tuple to be used with the query.predexp method");

PyDoc_STRVAR(integer_var_doc,
"integer_var(value) -> (predexp_code, value)\n\
\n\
Creates a tuple to be used with the query.predexp method");

PyDoc_STRVAR(string_var_doc,
"string_var(value) -> (predexp_code, value)\n\
\n\
Creates a tuple to be used with the query.predexp method");

PyDoc_STRVAR(geojson_var_doc,
"geojson_var(value) -> (predexp_code, value)\n\
\n\
Creates a tuple to be used with the query.predexp method");
...
Creates a tuple to be used with the query.predexp method");
/* No arg Docstrings */
PyDoc_STRVAR(rec_device_size_doc,
"rec_device_size() -> (predexp_code,)\n\
\n\
Creates a tuple to be used with the query.predexp method");

PyDoc_STRVAR(rec_last_update_doc,
"rec_last_update() -> (predexp_code,)\n\
\n\
Creates a tuple to be used with the query.predexp method");

PyDoc_STRVAR(rec_void_time_doc,
"rec_void_time() -> (predexp_code,)\n\
\n\
Creates a tuple to be used with the query.predexp method");

PyDoc_STRVAR(integer_equal_doc,
"integer_equal() -> (predexp_code,)\n\
\n\
Creates a tuple to be used with the query.predexp method");

PyDoc_STRVAR(integer_unequal_doc,
"integer_unequal() -> (predexp_code,)\n\
\n\
Creates a tuple to be used with the query.predexp method");

PyDoc_STRVAR(integer_greater_doc,
"integer_greater() -> (predexp_code,)\n\
\n\
Creates a tuple to be used with the query.predexp method");

PyDoc_STRVAR(integer_greatereq_doc,
"integer_greatereq() -> (predexp_code,)\n\
\n\
Creates a tuple to be used with the query.predexp method");

PyDoc_STRVAR(integer_less_doc,
"integer_greater() -> (predexp_code,)\n\
\n\
Creates a tuple to be used with the query.predexp method");

PyDoc_STRVAR(integer_lesseq_doc,
"integer_greatereq() -> (predexp_code,)\n\
\n\
Creates a tuple to be used with the query.predexp method");

PyDoc_STRVAR(string_equal_doc,
"string_equal() -> (predexp_code,)\n\
\n\
Creates a tuple to be used with the query.predexp method");

PyDoc_STRVAR(string_unequal_doc,
"string_unequal() -> (predexp_code,)\n\
\n\
Creates a tuple to be used with the query.predexp method");

PyDoc_STRVAR(geojson_contains_doc,
"geojson_contains_doc() -> (predexp_code,)\n\
\n\
Creates a tuple to be used with the query.predexp method");

PyDoc_STRVAR(geojson_within_doc,
"geojson_within_doc() -> (predexp_code,)\n\
\n\
Creates a tuple to be used with the query.predexp method");
...
static PyMethodDef AerospikePredExp_Methods[] = {
	...
	OneArgPredExpFunctionEntry(RecDigestModulo, rec_digest_modulo, rec_digest_modulo_doc),

	OneArgPredExpFunctionEntry(ListIterateOr, list_iterate_or, list_iterate_or_doc),
	OneArgPredExpFunctionEntry(ListIterateAnd, list_iterate_and, list_iterate_and_doc),
	OneArgPredExpFunctionEntry(MapkeyIterateOr, mapkey_iterate_or, mapkey_iterate_or_doc),
	OneArgPredExpFunctionEntry(MapkeyIterateAnd, mapkey_iterate_and, mapkey_iterate_and_doc),
	OneArgPredExpFunctionEntry(MapvalIterateOr, mapval_iterate_or, mapval_iterate_or_doc),
	OneArgPredExpFunctionEntry(MapvalIterateAnd, mapval_iterate_and, mapval_iterate_and_doc),

	/* No argument Predexp functions */
	NoArgPredExpFunctionEntry(PredexpNot, predexp_not, predexp_not_doc),
	NoArgPredExpFunctionEntry(RecDeviceSize, rec_device_size, rec_device_size_doc),
	NoArgPredExpFunctionEntry(RecLastUpdate, rec_last_update, rec_last_update_doc),
	...
	NoArgPredExpFunctionEntry(IntegerUnequal, integer_unequal, integer_unequal_doc),
	NoArgPredExpFunctionEntry(IntegerGreater, integer_greater, integer_greater_doc),
	NoArgPredExpFunctionEntry(IntegerGreaterEq, integer_greatereq, integer_greatereq_doc),
	NoArgPredExpFunctionEntry(IntegerLess, integer_less, integer_less_doc),
	NoArgPredExpFunctionEntry(IntegerLessEq, integer_lesseq, integer_lesseq_doc),
	NoArgPredExpFunctionEntry(StringEqual, string_equal, string_equal_doc),
	NoArgPredExpFunctionEntry(StringUnequal, string_unequal, string_unequal_doc),
	NoArgPredExpFunctionEntry(GeojsonContains, geojson_contains, geojson_contains_doc),
	NoArgPredExpFunctionEntry(GeojsonWithin, geojson_within, geojson_within_doc),
	/* String regex is special so it gets a non macro definition */
	{"string_regex", (PyCFunction)AerospikePredExp_PredexpStringRegex, METH_VARARGS,string_regex_doc},
	{NULL, NULL, 0, NULL}
};
...
{
	PyObject * module;
	MOD_DEF(module, "aerospike.predexp", "Query Predexp", AerospikePredExp_Methods);
	return module;
}
