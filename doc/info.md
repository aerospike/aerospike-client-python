...
aerospike.Client.info - sends an info request to a single cluster node.

## Description

```
response = aerospike.Client.info ( request )

```

**aerospike.Client.info()** returns a *response* for a particular *request* string
