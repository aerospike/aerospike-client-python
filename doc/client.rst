...
.. class:: Client
    ...
    .. method:: abort(transaction: aerospike.Transaction) -> int
        ...
        :raises: :py:exc:`~aerospike.exception.TransactionFailed` if a commit already failed
            ...
            transaction forward, so abort is refused; check
            ``err.abort_status == aerospike.ABORT_COMMIT_FAILED`` (equivalently,
            ``transaction.state == aerospike.TXN_STATE_COMMIT_FAILED``), then call
