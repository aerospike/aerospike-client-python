...
class TestMRTBasicFunctionality:
    ...
    def insert_or_update_records(self):
        ...
        for i, key in enumerate(self.keys):
            self.client.put(key, {self.bin_name: i})
    ...
    def test_commit_api_and_functionality(self, get_commit_status: bool):
        ...
        self.client.put(self.keys[0], {self.bin_name: 1}, policy=policy)
        # Reads in an MRT should read the intermediate values of the MRT
        _, _, bins = self.client.get(self.keys[0], policy)
        # Check that original value was overwritten
        assert bins == {self.bin_name: 1}
        self.client.put(self.keys[1], {self.bin_name: 2}, policy=policy)

        retval = self.client.commit(transaction=mrt, get_commit_status=get_commit_status)
        ...
        for i in range(len(self.keys)):
            _, _, bins = self.client.get(self.keys[i])
    ...
    def test_abort_api_and_functionality(self, get_abort_status: bool):
        ...
        retval = self.client.abort(transaction=mrt, get_abort_status=get_abort_status)
        if get_abort_status:
            assert type(retval) is int
        else:
            assert retval is None
    ...
    def test_abort_fail(self):
        ...
        self.client.put(self.keys[0], {self.bin_name: 1}, policy=policy)
        self.client.commit(mrt)
        with pytest.raises(e.RollAlreadyAttempted):
            self.client.abort(mrt)
