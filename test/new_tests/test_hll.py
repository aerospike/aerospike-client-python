...
class TestHLL(object):
        ...
        for x in range(5):
            ...
            ops = [
                ...
                hll_operations.hll_add(
                    "hll_bin", ["key%s" % str(i) for i in range(x + 1)], 8
                ),
                hll_operations.hll_add(
                    "mh_bin", ["key%s" % str(i) for i in range(x + 1)], 6, 12
                ),
                hll_operations.hll_add(
                    "hll_binl", ["key%s" % str(i) for i in range(100)], 8
                ),
                hll_operations.hll_add(
                    "hll_binlh", ["key%s" % str(i) for i in range(50)], 8
                ),
                hll_operations.hll_add(
                    "hll_bin_big", ["key%s" % str(i) for i in range(1000)], 10
                ),
                hll_operations.hll_add(
                    "hll_binu", ["key6", "key7", "key8", "key9", "key10"], 10
                ),
            ]
    ...
    def test_pos_hll_add(self, policy, expected_result):
        ...
        ops = [
            hll_operations.hll_add(
                "new_bin", ["key1", "key2", "key3"], index_bit_count=8, policy=policy
            )
        ]
    ...
    def test_neg_hll_add_mh(self, policy, expected_result):
        ...
        ops = [
            hll_operations.hll_add(
                "hll_binl", ["key101", "key102", "key103"], 6, 8, policy
            )
        ]
    ...
    @pytest.mark.parametrize(
        "index_bits, bin, expected_result",
        [(2, "hll_bin", e.InvalidRequest), (4, "mh_bin", e.OpNotApplicable)],
    )
    ...
    def test_pos_hll_get_intersect_count(self):
        ...
        records = [
            record[2]["hll_binu"]
            for record in self.as_connection.get_many(self.test_keys[:1])
        ]
        ...
        assert (
            res["hll_binl"] >= actual_intersect - rel_error
            and res["hll_binl"] <= actual_intersect + rel_error
        )
    ...
    def test_pos_hll_get_intersect_count_mh(self):
        ...
        records = [
            record[2]["mh_bin"]
            for record in self.as_connection.get_many(self.test_keys[:4])
        ]
    ...
    @pytest.mark.parametrize(
        "hll_bins, bin, expected_result", [(5, "hll_binl", e.InvalidRequest)]
    )
    def test_neg_hll_get_intersect_count(self, hll_bins, bin, expected_result):
        ...
        records = [
            record[2][bin]
            for record in self.as_connection.get_many(self.test_keys[: hll_bins - 1])
        ]
    ...
    def test_pos_hll_get_similarity(self):
        ...
        _, _, res = self.as_connection.operate(self.test_keys[4], ops)
        assert (
            res["hll_binlh"] >= actual_similarity - rel_error
            and res["hll_binlh"] <= actual_similarity + rel_error
        )
    ...
    def test_pos_hll_get_union(self):
        ...
        records = [
            record[2]["hll_binu"]
            for record in self.as_connection.get_many(self.test_keys)
        ]
