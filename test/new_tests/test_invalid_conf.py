...
class TestInvalidClientConfig(object):
    ...
    def test_lua_user_path_too_long(self):
        with pytest.raises(e.ParamError) as err:
            aerospike.client(
                {"hosts": [("localhost", 3000)], "lua": {"user_path": "a" * 256}}
            )
    ...
    def test_non_callable_serializer(self):
        with pytest.raises(e.ParamError) as err:
            aerospike.client(
                {"hosts": [("localhost", 3000)], "serialization": (5, lambda x: 5)}
            )
    ...
    def test_non_callable_deserializer(self):
        with pytest.raises(e.ParamError) as err:
            aerospike.client(
                {"hosts": [("localhost", 3000)], "serialization": (lambda x: 5, 5)}
            )
    ...
    def test_negative_threshold_value(self):
        with pytest.raises(e.ParamError) as err:
            aerospike.client(
                {"hosts": [("localhost", 3000)], "compression_threshold": -1}
            )
    ...
    def test_invalid_subpolicy_base_types(self, policy, key, value):
        ...
        with pytest.raises(e.ParamError):
            aerospike.client(
                {"hosts": [("localhost", 3000)], "policies": {policy: subpolicy}}
            )
    ...
    def test_invalid_read_policy_types(self, key, value):
        ...
        with pytest.raises(e.ParamError):
            aerospike.client(
                {"hosts": [("localhost", 3000)], "policies": {"read": subpolicy}}
            )
    ...
    def test_invalid_write_policy_types(self, key, value):
        ...
        with pytest.raises(e.ParamError):
            aerospike.client(
                {"hosts": [("localhost", 3000)], "policies": {"write": subpolicy}}
            )
    ...
    def test_invalid_operat_policy_types(self, key, value):
        ...
        with pytest.raises(e.ParamError):
            aerospike.client(
                {"hosts": [("localhost", 3000)], "policies": {"operate": subpolicy}}
            )
    ...
    def test_invalid_batch_policy_types(self, key, value):
        ...
        with pytest.raises(e.ParamError):
            aerospike.client(
                {"hosts": [("localhost", 3000)], "policies": {"batch": subpolicy}}
            )

    @pytest.mark.parametrize(
        "key, value", [("durable_delete", "durable")]
    )  # should be bool
    def test_invalid_scan_policy_types(self, key, value):
        subpolicy = {key: value}
        with pytest.raises(e.ParamError):
            aerospike.client(
                {"hosts": [("localhost", 3000)], "policies": {"scan": subpolicy}}
            )
    ...
    def test_invalid_query_policy_types(self, key, value):
        ...
        with pytest.raises(e.ParamError):
            aerospike.client(
                {"hosts": [("localhost", 3000)], "policies": {"query": subpolicy}}
            )
    ...
    def test_invalid_apply_policy_types(self, key, value):
        ...
        with pytest.raises(e.ParamError):
            aerospike.client(
                {"hosts": [("localhost", 3000)], "policies": {"apply": subpolicy}}
            )
    ...
    def test_invalid_remove_policy_types(self, key, value):
        ...
        with pytest.raises(e.ParamError):
            aerospike.client(
                {"hosts": [("localhost", 3000)], "policies": {"remove": subpolicy}}
            )
