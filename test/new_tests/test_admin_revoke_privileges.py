...
import aerospike
from .conftest import poll_until_role_doesnt_exist, admin_create_user_and_poll(self.client, 
...
class TestRevokePrivilege(TestBaseClass):
    ...
    def test_revoke_privileges_incorrect_role_type(self):
        ...
        """
        try:
            self.client.admin_revoke_privileges(1, [{"code": aerospike.PRIV_USER_ADMIN}])
    ...
    def test_revoke_privileges_unknown_privilege_type(self):
        ...
        """
        try:
            self.client.admin_revoke_privileges("usr-sys-admin-test", [{"code": 64}])
