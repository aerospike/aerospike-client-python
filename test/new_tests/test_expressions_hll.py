...
from aerospike_helpers.expressions import And, Eq, GE, GT, HLLAdd, HLLBin, HLLDescribe, HLLGetCount, HLLGetIntersectCount, HLLGetSimilarity, HLLGetUnion, HLLGetUnionCount, HLLInit, HLLMayContain, LE
...
class TestExpressions(TestBaseClass):
    ...
    def test_hll_add_pos(self, policy, listp, index_bc, mh_bc, bin, expected):
        ...
        expr = And(
            GE(HLLGetCount(HLLAdd(policy, listp, index_bc, mh_bc, bin)), lower_lim),
            LE(HLLGetCount(HLLAdd(policy, listp, index_bc, mh_bc, bin)), upper_lim),
        )
    ...
    def test_hll_init_pos(self, policy, index_bc, mh_bc, bin, expected):
        ...
        ops = [expressions.expression_read(bin, expr.compile())]
    ...
    def test_hll_get_union_count_pos(self, bin, expected):
        ...
        expr = And(
            GT(HLLGetUnionCount(records, bin), lower_lim),
            LE(HLLGetUnionCount(records, bin), upper_lim),
        )
    ...
    def test_hll_get_intersect_count_pos(self, bin, expected):
        ...
        expr = And(
            GE(HLLGetIntersectCount(records, bin), lower_lim),
            LE(HLLGetIntersectCount(records, bin), upper_lim),
        )
    ...
    def test_hll_get_similarity_pos(self, bin, expected):
        ...
        records = [record[2]["hll_bin2"]]
        expr = And(
            GE(HLLGetSimilarity(records, bin), expected - 0.03),
            LE(HLLGetSimilarity(records, bin), expected + 0.03),
        )
