...
class TestQueryPagination(TestBaseClass):
    ...
    def test_query_pagination_with_existent_ns_and_set(self):
        ...
        assert len(records) == ((query_page_size[0] * query_pages[0]) if (query_page_size[0] * query_pages[0]) < max_records else max_records)
    ...
    def test_query_pagination_with_existent_ns_and_none_set(self):
        ...
        all_records = self.partition_1000_count + \
            self.partition_1001_count + \
            self.partition_1002_count + \
            self.partition_1003_count
        ...
        for i in range(num_populated_partitions):
            query_obj.foreach(callback, {'partition_filter': {
                            'begin': 1000, 'count': num_populated_partitions},
    ...
    def test_query_pagination_with_max_records_policy(self):
        ...
        query_obj.foreach(callback, {'max_records': max_records, 'partition_filter': {
                         'begin': 1000, 'count': 1}})
    ...
    def test_query_pagination_with_callback_contains_error(self):
        ...
        with pytest.raises(e.ClientError) as err_info:
            query_obj.foreach(callback, {'timeout': 1000, 'partition_filter': {
                             'begin': 1001, 'count': 1}})
