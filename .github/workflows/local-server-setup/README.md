...
time it gets run.
- The client certificate and private key is also embedded here so the integration tests can reuse them when mutual authentication is enabled.
- We support TLS in order to check that the OpenSSL library bundled with our wheels are working properly. You can
manually verify this by enabling debug logging in the client and checking that the server certificate was verified
